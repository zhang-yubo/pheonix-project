...
//control and display booleans
boolean GPSECHO = false;
boolean Display = true;
boolean Motor = false;

//booleans that check for data
...
void loop()
{
  listenCommand(); //check for and read any user inputs from terminal
  ...
  if (Serial2.available()) //input from Moteino
  {
    ...
    { 
      ...
      {
        Serial.println("Error: Data type IDs not detected");
        Serial.print("Detected IDs: "); Serial.println(rocketData.ID);
        for (int i=0; i<sizeof(buff); i++) //echo received sentence
        {
          Serial.print(buff[i]);
        }
      }
...
void listenCommand()
{
  if (Serial.available() > 0) //input commands from terminal
  {
    ...
    if (input == 'G') //toggle display of raw NMEA sentences from ground GPS
      GPSECHO = !GPSECHO;
...
void motorCommand()
{
  double lonDiff = (groundGPS.doubleFour - rocketGPS.doubleFour)*1110;
  double latDiff = (rocketGPS.doubleThree - groundGPS.doubleThree)*887; //reduce digits
  double azimuth = 180;

  
  double elevation = atan2 ((rocketGPS.doubleFive - groundGPS.doubleFive)*0.305, pow(pow(latDiff,2)+pow(lonDiff,2),0.5));
...
{      
    ...
    sentence += (int)rocketGPS.doubleThree;
    sentence += ".";
    if ((int)((rocketGPS.doubleThree - (int)rocketGPS.doubleThree)*1e5)<1e5)
      sentence += "0";
    sentence += (int)((rocketGPS.doubleThree - (int)rocketGPS.doubleThree)*1e5);
    sentence += ",";
    
    sentence += g_GPS.lat;sentence += ",";
    sentence += (int)rocketGPS.doubleFour;
    sentence += ".";
    if ((int)((rocketGPS.doubleFour - (int)rocketGPS.doubleFour)*1e5)<1e5)
      sentence += "0";
    sentence += (int)((rocketGPS.doubleFour - (int)rocketGPS.doubleFour)*1e5);
    sentence += ",";
    
    sentence += g_GPS.lon;sentence += ",";
    sentence += rocketGPS.doubleSix;sentence += ",";
    sentence += "100,";
...
//    Serial.print(rocketGPS.doubleThree,7);Serial.print(",");
//    Serial.print(g_GPS.lat);Serial.print(",");
//    Serial.print(rocketGPS.doubleFour,7);Serial.print(",");
